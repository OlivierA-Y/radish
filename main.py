...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--mode", choices=["synthetic", "tcga", "brats", "manifest"],
...
def run_pipeline(args: argparse.Namespace) -> None:
    ...
    for subj_data in raw_subjects:
        ...
        try:
            ...
            if args.mode == "synthetic":
                ...
                clinical   = subj_data.get("clinical", {})
            ...
            save_json(json_str, features_dir / f"{sid}.json")
    ...
    # ── 4. Evaluate ───────────────────────────────────────────────────────
    if all_predictions:
        save_predictions_csv(all_predictions, out_dir / "predictions.csv")
        save_reasoning_traces(all_predictions, out_dir / "reasoning_traces.txt")
        logger.info("Saved predictions to %s", out_dir / "predictions.csv")
        logger.info("Saved reasoning traces to %s", out_dir / "reasoning_traces.txt")
